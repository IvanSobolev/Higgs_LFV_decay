...
/* Is now obsolete, is equal to four1.
...
#if 0
double erff(double x); /* Present in mathcalls.h... */
#endif
...
/* WARNING !!! name changed from standard (fmin) ! */
...
void quadvl(double x, double y, double *fa, double *fb, double *fc,
	    double *fd);
...
/* WARNING ! Name changed from "select" not to conflict with sys/time.h. */
...
void sprstm(double sa[], unsigned long ija[], double sb[], unsigned long ijb[],
	    ...
	    unsigned long ijc[]);
void sprstp(double sa[], unsigned long ija[], double sb[],
	    unsigned long ijb[]);
...
void tptest(double data1[], double data2[], unsigned long n,
